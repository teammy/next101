...
const ksh_conn_pool = mysql.createPool({
...
const e_leave_conn_pool = mysql.createPool({
...
export async function query_ksh(sql, params) {
  const [rows] = await ksh_conn_pool.query(sql, params);
  return rows;
}

export async function query_eleave(sql, params) {
  const [rows] = await e_leave_conn_pool.query(sql, params);
  return rows;
}